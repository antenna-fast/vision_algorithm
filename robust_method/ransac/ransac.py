...
def ransac_line(pts, sample_num):
    ...
    confidence = 0  # 模型置信度
    inlier_buff = []  # a set in {nx2}
    outlier_buff = []  # a set in {nx2}
    # 内外点阈值
    inlier_threshold = 0.5
    # 通过最佳模型去除外点
    for pt_idx in range(data_len):  # all point!  这里可以直接写成矩阵型形式
        pt = pts[pt_idx]
...
# 迭代一次后，基本能找到离群点
def ransac_pose(source, target, iter_num):
    # ransan 随机采样一致性算法，改进：记录变化趋势，减少随机性
    # 基本思想
    # 1.从所有的数据中进行随机采样，得到一组足以建模的一组数据
    # 2.判断模型质量
    ...
    while iter_num > 0:
        ...
        # 2.从以上样本估计模型 -- ICP  迭代几次看看
        W = eye(len(sub_source))

        for iter_num in range(1):  # ICP迭代次数
        ...
        source_trans = dot(r_mat_res, (source + t_res).T).T
        dist = norm(source_trans - target)
