...
if __name__ == '__main__':
    ...
    # 这里全是0.1噪声的  遍历所有环  每个噪声的都可以形成一幅图
    noise_rate = 0.1
    scale_ratio = 1  # 尺度因子
    ...
    repeat_buff = []
    for k in k_list:
        # if noise_rate == 0.1:  # 0.1的时候文件名没写...0.1
        #     key_pts_buff_1 = loadtxt('../save_file/key_pts_buff_1_k_' + str(k) + '.txt')  # Ground Truth
        #     key_pts_buff_2 = loadtxt('../save_file/key_pts_buff_2_k_' + str(k) + '.txt')
        #
        # else:
        key_pts_buff_1 = loadtxt('../save_file/key_pts_buff_1_k_' + str(k) + '_' + str(noise_rate) + '.txt')  # Ground Truth
        key_pts_buff_2 = loadtxt('../save_file/key_pts_buff_2_k_' + str(k) + '_' + str(noise_rate) + '.txt')
        ...
        print('k:{0}  噪声: {1:.2f}   重复率:{2:.3f}'.format(k, noise_rate, ra))

        repeat_buff.append(ra)

    repeat_buff = array(repeat_buff)
    savetxt('../save_file/repeat_buff_noise_' + str(noise_rate) + '.txt', repeat_buff)
